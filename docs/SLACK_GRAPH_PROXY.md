...
`/api/graph` proxies the internal nagiosgraph CGI from this app's own public host (`statuspage.pgmac.net.au`), which has no Access gate. To avoid becoming an open proxy, requests must carry a valid HMAC-SHA256 signature and only three parameters are ever forwarded upstream.

## Endpoint

```
GET /api/graph?host=<host>&service=<service>&period=<period>&expires=<unix-ts>&sig=<hex>
```

- `period` must be one of `day`, `week`, `month`, `quarter`, `year` — anything else is rejected before the signature is even checked.
- `sig` = `HMAC-SHA256(secret, "{host}|{service}|{period}|{expires}")`, hex-encoded.
...
params = sign_graph_params("web01", "HTTPS", "day", secret, ttl_seconds=604800)
# {"host": ..., "service": ..., "period": ..., "expires": ..., "sig": ...}
```
...
payload="${host}|${service}|${period}|${expires}"
